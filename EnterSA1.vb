Public Class EnterSA1
        ...
        Else
            ...
            Else
                For j = 1 To StudentsRecord.Count
                    Dim NewAssessmentRecord As New Assessment With {.SA1 = 0, .Class = ComboClass.Text, .Total2 = 0, .Full_Name = StudentsRecord.ToList(j - 1).Full_Name, .Registration_No = StudentsRecord.ToList(j - 1).Registration_No, .Stream = ComboStream.Text, .Subject = ComboSubject.Text}
    ...
    Private Sub ButtonEnter_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ButtonEnter.Click
        ...
        If RecordPosition > (AssessmentRecord.Count - 1) Then
            ...
            Exit Sub
        End If
        AssessmentRecord.ToList(RecordPosition).SA1 = Val(TextScore.Text)
        db.SubmitChanges()
        RecordPosition += 1
